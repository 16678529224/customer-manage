...
(function () {
    ...
    function l() {
        ...
        var w = if_check_slide_passcode_token;
        var x = {
    ...
    function f(y, x) {
        ...
        if (y == 0) {
            clearTimeout(n);
            if (w) {
                w.unbind("click").bind("click", qr_submitClickEvent);
                w.removeClass("btn92").addClass("btn92s")
            }
            return
        } else {
            y--
    ...
    jQuery.extend({
        whatsSelect: function (y) {
            if (y) {
                var w = $('#normal_passenger_id input[type="checkbox"]:checked').length;
                if ($("#dj_passenger_id li").length == 0) {
                    return true
                }
                return w > 0 ? true : false
            } else {
                var x = $('#dj_passenger_id input[type="checkbox"]:checked').length;
    ...
    function d(V) {
        ...
        closeWin = function (X, W) {
            unLoadGrayBackground();
            ableClickSubmitButtonOrPreStepBUtton();
            if (dhxWins.isWindow(X + "_")) {
                dhxWins.window(X + "_").setModal(false);
                dhxWins.window(X + "_").hide();
                if (W) {
                    refreshImg("passenger", "randp")
                }
            }
        };
        m = function (ad, aa, X, W, Z) {
            var ac = '<div class="tit">' + (aa ? '<span class="colorC">' + ad + "</span>" : ad) + "</div>";
            var Y = "<P>" + X + "</p>";
            var ab = aa ? '<p>请点击[<a href="' + ctx + 'view/train_order.html?type=2"><strong>我的12306</strong></a>]办理其他业务。您也可以点击[<a href="' + ctx + 'leftTicket/init"><strong>预订车票</strong></a>]，重新规划您的旅程。</p>' : '<P>查看订单处理情况，请点击“<a href="' + ctx + 'view/train_order.html?type=1">未完成订单</a>”</P>';
            if (!aa && canInsurance) {
                ab += "<P style='font-size:16px;color:#333;'><strong>欢迎购买铁路乘意险</strong></p>"
            }
            $("#iamge_status_id").removeClass().addClass("icon i-" + Z);
            if (W) {
            ...
            $("#orderResultInfo_id").html(ac + (X == "" || X == null || X == "undefined" || X == undefined ? "" : Y) + ab);
            O("transforNotice_id");
            if (W) {
        ...
        function O(ab) {
            closeWin(ab, false);
            if ("checkticketinfo_id" == ab) {
                var Z = ticketInfoForPassengerForm.queryLeftNewDetailDTO;
                if (Z.to_station_telecode == ticket_submit_order.special_areas.lso || Z.to_station_telecode == ticket_submit_order.special_areas.dao || Z.to_station_telecode == ticket_submit_order.special_areas.ado || Z.to_station_telecode == ticket_submit_order.special_areas.nqo || Z.to_station_telecode == ticket_submit_order.special_areas.tho) {
                    if (P()) {
                        if ("Y" == canChooseBeds) {
            ...
            if (N() && canInsurance) {
                $("#notice_4_id").html('<strong><span style="color: #3177BF;font-size:12px;">*购买铁路乘意险的注册用户年龄须在18周岁以上，使用非二代居民身份证注册的用户如购买铁路乘意险，须在<a href="../view/information.html">“我的12306—个人信息”</a>如实填写“出生日期”。<br />*父母为未成年子女投保，须在<a href="../view/passengers.html">我的常用联系人</a>登记未成年子女的有效身份证件信息。</span></strong>')
            } else {
                $("#notice_4_id").html("")
            }
            if (transport_in_SF) {
            ...
            var X = ifAlertCode ? 250 : 200;
            var aa = D() + ($(window).height() / 2 - X);
            Y.setDimension($("#content_" + ab).width(), $("#content_" + ab).height() + 10);
            $(".dhtmlx_window_active").height($("#content_" + ab).height());
            $(".dhtmlx_window_active").css({left: W + "px", top: aa + "px"})
        }
        ...
        function H() {
            ...
            for (var X = 0; X < limit_tickets.length; X++) {
                var W = limit_tickets[X];
                if (!(W.seat_type == "F" || W.seat_type == "A")) {
                    return false
                }
            }
            return true
        ...
        function A() {
            for (var X = 0; X < limit_tickets.length; X++) {
                var W = limit_tickets[X];
                if ((W.ticket_type == ticket_submit_order.ticket_type.child || W.ticket_type == ticket_submit_order.ticket_type.adult) && W.id_type == "1" && (W.seat_type == "F" || W.seat_type == "A")) {
                    if (Number(W.id_no.substr(17, 18)) % 2 != 0) {
                        return false
                    }
                } else {
                    return false
        ...
        function N() {
            for (var X = 0; X < limit_tickets.length; X++) {
                var W = limit_tickets[X];
                if (W.ticket_type == ticket_submit_order.ticket_type.child) {
                    return true
                }
            }
            return false
        ...
        function C() {
            for (var X = 0; X < limit_tickets.length; X++) {
                var W = limit_tickets[X];
                if (W.ticket_type == ticket_submit_order.ticket_type.student) {
                    return true
                }
            }
            return false
        ...
        function P() {
            for (var X = 0; X < limit_tickets.length; X++) {
                var W = limit_tickets[X];
                if ((ticketInfoForPassengerForm.tour_flag == ticket_submit_order.tour_flag.fc || ticketInfoForPassengerForm.tour_flag == ticket_submit_order.tour_flag.gc) && W.save_status != "" && (W.id_type == ticket_submit_order.passenger_card_type.passport || W.id_type == ticket_submit_order.passenger_card_type.work || W.id_type == ticket_submit_order.passenger_card_type.taiwan)) {
                    return true
                } else {
                    if ((ticketInfoForPassengerForm.tour_flag == ticket_submit_order.tour_flag.wc || ticketInfoForPassengerForm.tour_flag == ticket_submit_order.tour_flag.dc) && (W.id_type == ticket_submit_order.passenger_card_type.passport || W.id_type == ticket_submit_order.passenger_card_type.work || W.id_type == ticket_submit_order.passenger_card_type.taiwan)) {
                        return true
                    }
                }
            }
        ...
        function J() {
            if (ticketInfoForPassengerForm.isAsync == ticket_submit_order.request_flag.isAsync) {
                B(ticketInfoForPassengerForm.tour_flag)
            } else {
                L(ticketInfoForPassengerForm.tour_flag)
            }
        }
        ...
        function U() {
            var W = $("#qr_submit_id");
            W.unbind("click");
            W.removeClass("btn92s").addClass("btn92")
        }
        ...
        qr_submitClickEvent = function () {
            ...
            var W = j();
            if (W.length == 0 || limit_tickets.length == W.length / 2) {
        ...
        submitOrderClickEvent_common = function () {
            var Z = $('#normal_passenger_id input[type="checkbox"]:checked').length;
            var ab = $('#dj_passenger_id input[type="checkbox"]:checked').length;
            if ($("#normal_passenger_id").length > 0 && Z == 0 && ab == 0) {
                dhtmlx.alert({title: "温馨提示", ok: "确定", text: "请选择乘车人！", type: "alert-error"});
                return
            }
            if (isDw == "Y" && $("#chooseAllDW").is(":checked")) {
                var Y = $("#ticketInfo_id").find("select[id^=seatType_]");
                var X = Y.eq(0).val();
                if (X != "A" && X != "F") {
                ...
                for (var W = 1, aa = Y.length; W < aa; W++) {
                    var ac = Y.eq(W);
                    if (ac.val() != X) {
                        alertWarningMsgByTit_header("提示", dwLimitErrorStStr);
                        $("#i-okmypasscode1").hide();
        ...
        function y(X) {
            var W = dhxWins.createWindow(X + "_", 0, 0, 660, 100);
            W.attachObject(X);
            W.clearIcon();
            W.denyResize();
            W.setModal(true);
        ...
        function L(W) {
            ...
            if (W == ticket_submit_order.tour_flag.dc) {
                Y = ctx + "confirmPassenger/confirmSingle";
                Z.dwAll = "N";
                if ($("#chooseAllDW")[0] && $("#chooseAllDW").is(":checked")) {
                    Z.dwAll = "Y"
                }
            } else {
                if (W == ticket_submit_order.tour_flag.wc) {
                    Y = ctx + "confirmPassenger/confirmGo"
                } else {
                    if (W == ticket_submit_order.tour_flag.fc) {
                        Y = ctx + "confirmPassenger/confirmBack";
                        Z.fczk = $("#fczk").is(":checked") ? "Y" : "N"
                    } else {
                        if (W == ticket_submit_order.tour_flag.gc) {
                            Y = ctx + "confirmPassenger/confirmResign"
                        } else {
                            m("订票失败!", true, "原因： 旅程形式为<span style='color:red'>" + W + "</sapn>,非法的旅程方式", true, "lose");
                            return
                        }
                    }
                }
            ...
            $.ajax({
                url: Y, type: "post", data: Z, dataType: "json", async: true, success: function (aa) {
                    if (aa.status) {
                        if (aa.data.submitStatus) {
                            otsRedirect("post", ctx + "payOrder/init?random=" + new Date().getTime(), {})
                        } else {
                            m("出票失败!", false, "原因： " + aa.data.errMsg + '<a  id="xg_close_win_id">点击修改</a>', false, "lose");
        ...
        function B(W) {
            ...
            if (W == ticket_submit_order.tour_flag.dc) {
                Y = ctx + "confirmPassenger/confirmSingleForQueue";
                Z.dwAll = "N";
                if ($("#chooseAllDW")[0] && $("#chooseAllDW").is(":checked")) {
                    Z.dwAll = "Y"
                }
                ...
                if (W == ticket_submit_order.tour_flag.wc) {
                    Y = ctx + "confirmPassenger/confirmGoForQueue"
                } else {
                    if (W == ticket_submit_order.tour_flag.fc) {
                        Y = ctx + "confirmPassenger/confirmBackForQueue";
                        Z.fczk = $("#fczk").is(":checked") ? "Y" : "N"
                    } else {
                        if (W == ticket_submit_order.tour_flag.gc) {
                            Y = ctx + "confirmPassenger/confirmResignForQueue"
                        } else {
                            m("订票失败!", true, "原因： 旅程形式<span style='color:red'>" + W + "</sapn>为非法的旅程方式", true, "lose");
                            return
                        }
                    }
                }
            ...
            $.ajax({
                url: Y, data: Z, type: "POST", dataType: "json", success: function (aa) {
                    if (aa.status) {
                        if (!aa.data.submitStatus) {
                            m("出票失败!", false, "原因： " + aa.data.errMsg + '<a id="xg_close_win_id" >点击修改</a>', false, "lose");
                            $("#xg_close_win_id").click(function () {
                            ...
                            var ab = new OrderQueueWaitTime(W, w, R);
                            ab.start()
                        }
                    } else {
                        m("订票失败!", true, "很抱歉！请关闭窗口重新预定车票", true, "lose")
        ...
        function w(W, Y, X) {
            if (Y <= 5) {
                ...
                if (Y > 30 * 60) {
                    ...
                    m("订单已经提交，最新预估等待时间" + X + "，请耐心等待。", false, "", false, "queue")
                }
            }
        }
        ...
        function R(W, Z, X) {
            if (Z == -1 || Z == -100) {
                var Y = "";
                if (W == ticket_submit_order.tour_flag.dc) {
                    Y = ctx + "confirmPassenger/resultOrderForDcQueue"
                } else {
                    if (W == ticket_submit_order.tour_flag.wc) {
                        Y = ctx + "confirmPassenger/resultOrderForWcQueue"
                    } else {
                        if (W == ticket_submit_order.tour_flag.fc) {
                            Y = ctx + "confirmPassenger/resultOrderForFcQueue"
                        } else {
                            if (W == ticket_submit_order.tour_flag.gc) {
                                Y = ctx + "confirmPassenger/resultOrderForGcQueue"
                            }
                        }
                    }
                }
                $.ajax({
                    url: Y,
                    data: {orderSequence_no: X.orderId},
                    type: "POST",
                    dataType: "json",
                    success: function (aa) {
                        if (aa.status) {
        ...
        function M(W, X) {
            if (X.name && X.card && X.tel) {
                closeWin("transforNotice_id", true);
                $("#608_check_msg").html(X.msg);
                dhtmlx.createWin({
                    winId: "608_check",
                    closeWinId: ["608_check_close", "608_check_cancel"],
                    okId: "608_check_ok",
                    okCallBack: function () {
                        $("#608_name").html(X.name);
                        $("#608_card").html(X.card);
                        $("#608_tel").html(X.tel);
                        $("#ticketInfo").html(X.ticketInfo);
                        dhtmlx.createWin({
                            winId: "608_complain",
                            closeWinId: ["608_complain_close", "608_complain_cancel"],
                            okId: "608_complain_ok",
                            okCallBack: function () {
                                var Y = dhtmlx.modalbox({
            ...
            if (W == -1) {
                ...
                if (W == -2) {
                    if (X.errorcode == 0) {
                        m("订票失败!", true, "原因： " + X.msg, false, "lose")
                    } else {
                        m("订票失败!", true, "原因： " + X.msg, false, "lose")
                    }
                } else {
        ...
        function K() {
            ...
            $.ajax({
                ...
                async: true,
                success: function (W) {
                    if (!W.data.submitStatus) {
                        if (W.data.isRelogin) {
                            window.location.href = ctx + "view/index.html?random=" + new Date().getTime()
                        } else {
                            if (W.data.isNoActive) {
                                m(W.data.errMsg, true, "", true, "warn")
                            } else {
                                if (W.data.checkSeatNum) {
                                    m("很抱歉，无法提交您的订单!", true, "原因： " + W.data.errMsg, true, "warn")
                                } else {
                                    m("出票失败!", true, "原因： " + W.data.errMsg, true, "warn")
                                }
                            }
                        }
                        return
                    }
                    intervalTime = W.data.ifShowPassCodeTime;
                    if (W.data.ifShowPassCode == "Y") {
                        doByAlert(true)
                    } else {
                        doByAlert(false)
                    }
                    canChooseSeats = W.data.canChooseSeats;
                    choose_Seats = W.data.choose_Seats;
                    canChooseBeds = W.data.canChooseBeds;
                    isCanChooseMid = W.data.isCanChooseMid;
                    if (W.data.smokeStr != "" && W.data.smokeStr.length > 0) {
                        $("#dialog_smoker_msg").html(W.data.smokeStr);
                        dhtmlx.createWin({
                            winId: "dialog_smoker",
        ...
        function Q(W) {
            $('div[dhxbox="1"]').hide();
            var X = W.data.get608Msg;
            if (undefined != X && typeof(X) != "undefined" && "" != X) {
                if (W.data.name && W.data.card && W.data.tel) {
                    $("#608_check_msg").html(X);
                    dhtmlx.createWin({
                        winId: "608_check",
                        closeWinId: ["608_check_close", "608_check_cancel"],
                        okId: "608_check_ok",
                        okCallBack: function () {
                            $("#608_name").html(W.data.name);
                            $("#608_card").html(W.data.card);
                            $("#608_tel").html(W.data.tel);
                            $("#ticketInfo").html(W.data.ticketInfo);
                            dhtmlx.createWin({
                                winId: "608_complain",
                                closeWinId: ["608_complain_close", "608_complain_cancel"],
                                okId: "608_complain_ok",
                    ...
                    dhtmlx.alert({
                        title: "警告", ok: "确定", text: X, type: "alert-error", callback: function () {
                            renderTickInfo(limit_tickets);
                            G(ticketInfoForPassengerForm.purpose_codes, W.data.isCheckOrderInfo, W.data.doneHMD)
                        }
                    })
                }
            } else {
        ...
        function F(W, X) {
            ...
            if (ticketInfoForPassengerForm.isAsync == ticket_submit_order.request_flag.isAsync && ticketInfoForPassengerForm.queryLeftTicketRequestDTO.ypInfoDetail != "") {
                $.ajax({
                    ...
                    success: function (Z) {
                        if (Z.status) {
                            if (Z.data.isRelogin == "Y") {
                                window.location.href = ctx + "view/index.html?random=" + new Date().getTime()
                            }
                            var aa = Z.data.ticket.split(",");
                            Y = "本次列车，" + (limit_tickets[0].seat_type_name).split("（")[0] + "余票";
                            if (parseInt(aa[0]) >= 0) {
                                Y += "<strong>" + aa[0] + "</strong>张"
                            } else {
                                Y += aa[0]
                            }
                            if (aa.length > 1) {
                            ...
                            Y += "。";
                            if (Z.data.op_2 == "true") {
                                Y += '<font color="red">目前排队人数已经超过余票张数，请您选择其他席别或车次。</font>';
                                $("#qr_submit_id").hide()
                            } else {
                                if (Z.data.countT > 0) {
                                    Y += '目前排队人数<font color="red">' + Z.data.countT + "</font>人，";
                                    Y += "<br/>请确认以上信息是否正确，点击“确认”后，系统将为您随机分配席位。"
                                }
                            }
                            var ab = $("#sy_ticket_num_id");
                            if (ab != null) {
                                ab.html(Y)
                            }
                            doTicketTitleShow(true);
                            renderCheckTickInfo(limit_tickets);
                            O("checkticketinfo_id");
                            if (parseInt(aa[0]) > 0 || parseInt(aa[1]) > 0 || "充足" == aa[0] || "充足" == aa[1]) {
                                T()
                            }
                        } else {
                            T()
    ...
    function u() {
        var w = $("span[id$='_check']");
        var y = true;
        for (var x = 0; x < w.length; x++) {
            if ($(w[x]).css("display") != "none") {
                y = false;
                break
            }
        }
    ...
    function q() {
        ...
        for (var A = 0; A < limit_tickets.length; A++) {
            var F = limit_tickets[A];
            G.push(F.ticket_type);
            if ((ticket_submit_order.tour_flag.wc == ticketInfoForPassengerForm.tour_flag || ticket_submit_order.tour_flag.dc == ticketInfoForPassengerForm.tour_flag) && ticket_submit_order.ticket_type.child != F.ticket_type) {
                if (jQuery.inArray(F.id_no + F.name, D) < 0) {
                    D.push(F.id_no + F.name)
                } else {
                    var B = ticketInfoForPassengerForm.queryLeftTicketRequestDTO.train_date.substr(0, 4) + "年" + ticketInfoForPassengerForm.queryLeftTicketRequestDTO.train_date.substr(4, 2) + "月" + ticketInfoForPassengerForm.queryLeftTicketRequestDTO.train_date.substr(6, 2) + "日";
                    m("出票失败", true, "互联网售票实行实名制：证件号<span style='color:red'><i><b>" + F.id_no + "</b></i></span>只能购买" + B + "车次" + ticketInfoForPassengerForm.queryLeftTicketRequestDTO.station_train_code + "的一张车票！", true, "warn");
                    return false
                }
            } else {
                if ((ticket_submit_order.tour_flag.wc == ticketInfoForPassengerForm.tour_flag || ticket_submit_order.tour_flag.dc == ticketInfoForPassengerForm.tour_flag) && F.save_status != "" && ticket_submit_order.ticket_type.child != F.ticket_type) {
                    if (jQuery.inArray(F.id_no + F.name, D) < 0) {
                        D.push(F.id_no + F.name)
                    } else {
                        var B = ticketInfoForPassengerForm.queryLeftTicketRequestDTO.train_date.substr(0, 4) + "年" + ticketInfoForPassengerForm.queryLeftTicketRequestDTO.train_date.substr(4, 2) + "月" + ticketInfoForPassengerForm.queryLeftTicketRequestDTO.train_date.substr(6, 2) + "日";
            ...
            if (F.save_status != "") {
                ...
                if (ticket_submit_order.tour_flag.gc == ticketInfoForPassengerForm.tour_flag) {
                    if (w.length > 0) {
                        if (jQuery.inArray(F.seat_type, w) < 0) {
                            m("出票失败", true, (CHANGETSFLAG == "Y" ? "变更到站" : "改签") + "时，必须选择相同席别", true, "warn");
                            return false
                        }
                    }
                    w.push(F.seat_type);
                    if (x.indexOf(F.seat_type) > -1) {
                        E.push(F.seat_type)
                    }
                    if (E.length > 1) {
                        m("出票失败", true, "卧铺不支持批量" + (CHANGETSFLAG == "Y" ? "变更到站" : "改签") + "，请单张" + (CHANGETSFLAG == "Y" ? "变更到站" : "改签") + "！", true, "warn");
                        return false
                    }
                }
            }
            if (F.ticket_type == ticket_submit_order.ticket_type.disability) {
                var y = id_type_code;
                if (y != ticket_submit_order.passenger_card_type.two) {
                    m("出票失败", true, "第 " + (A + 1) + " 张车票： 当前登录用户证件类型不是二代身份证，购买残疾军人（伤残警察）优待票需使用中华人民共和国居民身份证！", true, "warn");
                    return false
                } else {
                    if (F.id_type != ticket_submit_order.passenger_card_type.two) {
                        m("出票失败", true, "第 " + (A + 1) + " 张车票： 乘客证件类型不是二代身份证，购买残疾军人（伤残警察）优待票需使用中华人民共和国居民身份证！", true, "warn");
                        return false
                    }
                }
            }
            if (ticket_submit_order.tour_flag.wc == ticketInfoForPassengerForm.tour_flag || ticket_submit_order.tour_flag.dc == ticketInfoForPassengerForm.tour_flag) {
                if (F.ticket_type == ticket_submit_order.ticket_type.student) {
                    if (F.passenger_type != ticket_submit_order.passenger_type.student) {
                        var C = $.whatsSelect(false);
                        if (!C) {
                            m("出票失败", true, "第 " + (A + 1) + " 张车票： 乘客不是学生，请从常用联系人中选择学生购买此车票。", true, "warn");
                            return false
                        }
                    }
                }
    ...
    function v() {
        if (ticketInfoForPassengerForm.tour_flag == ticket_submit_order.tour_flag.wc || ticketInfoForPassengerForm.tour_flag == ticket_submit_order.tour_flag.dc || ticketInfoForPassengerForm.tour_flag == ticket_submit_order.tour_flag.fc) {
            ...
            for (var z = 0; z < limit_tickets.length; z++) {
                var w = limit_tickets[z].seat_type;
                var A = limit_tickets[z].ticket_type;
                if (A == ticket_submit_order.ticket_type.child) {
                    x.push(w)
                } else {
                    y += w
                }
    ...
    function g() {
        ...
        if (limit_tickets && limit_tickets.length > 0) {
            var A = "Y";
            var x = limit_tickets[0].seat_type;
            for (var z = 0; z < limit_tickets.length; z++) {
                var w = limit_tickets[z];
                if (w.seat_type != x) {
                    A = "N";
                    break
                }
            }
            if (canChooseSeats && "Y" == canChooseSeats && "Y" == A) {
                if (choose_Seats) {
                    var y = "*如果本次列车剩余席位无法满足您的选座需求，系统将自动为您分配席位。";
                    if ("Q" == x && choose_Seats.indexOf("Q") > -1) {
                        $("#id-seat-sel").css("display", "block");
                        $("#yideng1").css("display", "block");
                        if (limit_tickets.length > 1 && t() > 1) {
                            $("#yideng2").css("display", "block")
                        }
                        $("#notice_1_id").html(y)
                    }
                    if ("M" == x && choose_Seats.indexOf("M") > -1) {
                        $("#id-seat-sel").css("display", "block");
                        $("#yideng1").css("display", "block");
                        if (limit_tickets.length > 1 && t() > 1) {
                            $("#yideng2").css("display", "block")
                    ...
                    if ("O" == x && choose_Seats.indexOf("O") > -1) {
                        $("#id-seat-sel").css("display", "block");
                    ...
                    if ("P" == x && choose_Seats.indexOf("P") > -1) {
                        $("#id-seat-sel").css("display", "block");
                    ...
                    if ("9" == x && choose_Seats.indexOf("9") > -1) {
                        $("#id-seat-sel").css("display", "block");
                        $("#tedeng1").css("display", "block");
                        if (limit_tickets.length > 1 && t() > 1) {
                            $("#tedeng2").css("display", "block")
    ...
    function t() {
        var y = limit_tickets.length;
        for (var x = 0; x < limit_tickets.length; x++) {
            var w = limit_tickets[x];
            if (w.tour_flag == "gc" || w.tour_flag == "fc") {
                if (w.save_status == "") {
                    y--
                }
            }
    ...
    numSet = function (z, w) {
        var E = parseInt($("#x_no").text());
        var A = parseInt($("#z_no").text());
        var y = parseInt($("#s_no").text());
        var C = t();
        var x = E + A + y;
        if ("add" == z) {
            if (x < C) {
                var D = document.getElementById(w).innerText;
                var B = parseInt(D) + 1;
                document.getElementById(w).innerText = B;
                $("#selectBedNo").html(x + 1 + "/" + C)
            }
        } else {
            var D = document.getElementById(w).innerText;
            if (x > 0 && parseInt(D) > 0) {
...
(function () {
    ...
    $(document).ready(function () {
        ...
        if (ticketInfoForPassengerForm.tour_flag == ticket_submit_order.tour_flag.fc && $("#dw_fczk")[0]) {
            ...
            var aA = false;
            for (var aB = 0, aD = aC.length; aB < aD; aB++) {
                var aE = aC.eq(aB);
                var aF = aE.val();
                if (aF == "A" || aF == "F") {
                    aA = true;
    ...
    function ad() {
        $("#psInfo").mouseenter(function (aC) {
            var aA = aC.pageY + 10;
            var aB = aC.pageX;
            $(".srr-tips").eq(1).css({top: aA, left: aB});
            $(".srr-tips").eq(1).show()
        });
    ...
    function am() {
        $("#psInfo").mouseenter(function (aC) {
            var aA = aC.pageY + 10;
            var aB = aC.pageX;
            $(".srr-tips").eq(0).css({top: aA, left: aB});
            $(".srr-tips").eq(0).show()
        });
        $("#psInfo").mouseleave(function () {
            $(".srr-tips").hide()
    ...
    responseDjPassengerClick = function (aD) {
        ...
        if (aD.checked) {
            ...
            var aI = new Z(aH, "", "", aE, "", aA.passenger_name, aA.passenger_id_type_code, af(aA.passenger_id_no, aA.passenger_id_type_code, aA.passenger_id_type_name), aA.passenger_id_no, aA.allEncStr, aA.mobile_no, "", ticketInfoForPassengerForm.tour_flag, true, aA.passenger_type, false);
            if (!B(aI)) {
    ...
    responseNormalPassengerClick = function (aC) {
        ...
        if (aC.checked) {
            ...
            var aH = new Z(aG, "", "", aD, "", aK.passenger_name, aK.passenger_id_type_code, af(aK.passenger_id_no, aK.passenger_id_type_code, aK.passenger_id_type_name), aK.passenger_id_no, aK.allEncStr, aK.mobile_no, "", ticketInfoForPassengerForm.tour_flag, true, aK.passenger_type, false);
            if (!B(aH)) {
    ...
    function ap(aB) {
        ...
        $.templates({leftTableTemplate: aA});
    ...
    excuteClickEditPassenger = function (aA) {
        ...
        if (aA.attr("disabled") == "disabled") {
            ...
            if (aE.total_times == "91") {
                dhtmlx.alert({title: "修改常用联系人", ok: "确定", text: "不允许修改" + aE.passenger_name, type: "alert-error"});
                return
            }
    ...
    checkWinUpdatePassenger = function () {
        ...
        $.ajax({
            url: ctx + "passengers/editReal", type: "post", data: s, error: function (aF, aH, aG) {
                dhtmlx.modalbox.hide(aE);
                $("#dialog_update").show();
                $("#error_update_tr").show();
                $("#error_for_update_nameandidno").html("您的网络可能有问题").show()
            }, async: false, success: function (aF) {
                dhtmlx.modalbox.hide(aE);
                if (aF.data.flag) {
                    $("#error_for_update_nameandidno").hide();
    ...
    renderTickInfo = function (aI, aD) {
        ...
        $.templates({leftTableTemplate: aP});
    ...
    renderCheckTickInfo = function (aF) {
        ...
        $.templates({leftTableTemplate: aJ});
    ...
    function v() {
        $.ajax({
            type: "post", url: ctx + "confirmPassenger/getPassengerDTOs", async: true, success: function (aA) {
                if (aA.status) {
                    if (aA.data.isExist) {
                        ...
                        if (aB && "" != aB) {
                            dhtmlx.alert({title: "温馨提示", ok: "确定", text: aB, type: "alert-info"})
                        }
                    } else {
                    ...
                    if (aA.data.exMsg != "" && aA.data.exMsg != null && aA.data.exMsg != "null") {
                        dhtmlx.alert({title: "加载常用联系人", ok: "确定", text: aA.data.exMsg, type: "alert-error"});
                        return
                    }
                }
                ableClickSubmitButtonOrPreStepBUtton();
...
var ticket_submit_order = {
    ...
    seatType: {yz_type: "1"},
    ref_res_rules: {
        "4": [{refund_rule: "20%", res_rule: "免费", title: "bt_15d"}, {
            refund_rule: "30%",
            res_rule: "5%",
            title: "48h_15d"
        }, {refund_rule: "50%", res_rule: "10%", title: "24h_48H"}, {
            refund_rule: "不允许",
            res_rule: "20%",
            title: "lt_24h"
        }],
        "5": [{refund_rule: "10%", res_rule: "免费", title: "bt_15d"}, {
            refund_rule: "15%",
            res_rule: "5%",
            title: "48h_15d"
        }, {refund_rule: "25%", res_rule: "10%", title: "24h_48H"}, {
            refund_rule: "40%",
            res_rule: "20%",
            title: "lt_24h"
        }],
        "6": [{refund_rule: "10%", res_rule: "免费", title: "bt_15d"}, {
            refund_rule: "15%",
            res_rule: "5%",
            title: "48h_15d"
        }, {refund_rule: "25%", res_rule: "10%", title: "24h_48H"}, {
        ...
        "7": [{refund_rule: "5%", res_rule: "免费", title: "bt_15d"}, {
            refund_rule: "10%",
            res_rule: "5%",
            title: "48h_15d"
        }, {refund_rule: "15%", res_rule: "10%", title: "24h_48H"}, {
            refund_rule: "30%",
            res_rule: "20%",
            title: "lt_24h"
        }],
        "8": [{refund_rule: "5%", res_rule: "免费", title: "bt_15d"}, {
            refund_rule: "10%",
            res_rule: "5%",
            title: "48h_15d"
        }, {refund_rule: "15%", res_rule: "10%", title: "24h_48H"}, {
        ...
        "9": [{refund_rule: "免费", res_rule: "免费", title: "bt_15d"}, {
            refund_rule: "5%",
            res_rule: "5%",
            title: "48h_15d"
        }, {refund_rule: "10%", res_rule: "10%", title: "24h_48H"}, {
            refund_rule: "20%",
            res_rule: "20%",
            title: "lt_24h"
        }]
